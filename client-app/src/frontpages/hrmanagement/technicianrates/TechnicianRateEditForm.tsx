...
const TechnicianRateEditForm: React.FC<RouteComponentProps<DetailParams>> = ({
  match,
  history
}) => {
  ...
  useEffect(() => {
    loadTechnicianRate(match.params.id).then(
      () => initialFormState && setTechnicianRate(initialFormState)
    );
  ...
  const handleSubmit = () => { 
     editTechnicianRate(technicianRate).then(() => history.push(`/hrmanagement/technicianrate`));
  };
...
export default observer(TechnicianRateEditForm);
