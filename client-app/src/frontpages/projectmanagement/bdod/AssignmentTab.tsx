...
const AssignmentTab = () => {
  ...
  const handleSubmit = () => {
    tasktechniciansByCategory.map((tt) => assignTechnician(tt));
  };
          ...
          <Table.Body>
            {tasktechniciansByCategory.map((tasktechnician) => (
              <Table.Row key={tasktechnician.category}>
                <Table.Cell>
                  <Popup
                    trigger={<span>{tasktechnician.category}</span>}
                  ...
                  >
                    <Popup.Content>
                      <Table>
                        ...
                        <Table.Body>
                          {projecttasksByName.map(
                            (sorname) =>
                              sorname.itemCategory ===
                                tasktechnician.category && (
                                <Table.Row>
                                  <Table.Cell> {sorname.itemName} </Table.Cell>
                                  <Table.Cell>
                                    {" "}
                                    {sorname.itemDescription}{" "}
                                  </Table.Cell>
                                </Table.Row>
                              )
                          )}
                ...
                <Table.Cell>
                  <Form.Input
                    ...
                    onChange={(e, { name, value }) =>
                      updateMember(e, tasktechnician, name, value)
                    }
                  />
                </Table.Cell>
                <Table.Cell>
